...
```diagram
type: activity
title: Elección del backend de render

flow:
  - Se lee docviz.config.yaml
  - decision: ¿backend es kroki?
    yes:
      - decision: ¿la URL es local o está autorizada?
        yes:
          - Renderiza contra el Kroki propio
        no:
          - Aborta el build con ConfigError
    no:
      - Renderiza en el proceso local
  - Aplica el tema antes de dibujar
  - Sanea el SVG resultante
```
